...
export default function CategoryAdventure() {
  const [adventureCategory, setAdventureCategory] = useState(null); 
  ...
  const getGamesByCategory = async (category) => {
    try {
      const response = await axios.get(`${baseURL}/categories/Adventure`);
      setAdventureCategory(response.data);
  ...
  return (
    <div className="principalContainer_adventure">
      <div>
        <NavigateBar />
        <div>{<RightBar />}</div>
        <LeftBar />
        <div className="cardContainer">
          <div className="container cardSubContainer m-auto m-0 p-0 pb-5">
            <div className="text-center pt-5 ">
              ...
              <h1 className="textTitle pb-4 pt-5 ">ADVENTURE</h1>
            </div>

            <div className="container containerWrapperCard p-0 pt-5 m-auto">
              {adventureCategory ? (
                <div className="row">
                  {adventureCategory.games.map((game) => (
                    <div
                      key={game._id}
